var peNavScroll = (function() {
    ...
    var root = /firefox|trident/i.test(navigator.userAgent)
    ...
    var initialize = function(options) {
        ...
        function eP(elem) {
            if (
                getOffset(elem).top <
                root.scrollTop + tMenu.offsetHeight + config.offset + 2
            ) {
                return elem;
            }
        }
        ...
        var tMenu = document.querySelector(config.nav),
            links = config.allLinks
                ? document.querySelectorAll('a[href^="#"]')
                : tMenu.querySelectorAll("a"),
            navLinks,
            lId;

        function isRealLink(elem) {
            if (document.getElementById(/[^#]+$/.exec(elem.href)[0])) return elem;
        }
            ...
            if (document.getElementById(/[^#]+$/.exec(links[i].href)[0])) {
                links[i].addEventListener("click", function(e) {
                    ...
                    var endPos =
                        ...
                        (tMenu.offsetHeight + config.offset);
                    var scrollEndValue =
                        startPos + endPos < maxScroll ? endPos : maxScroll - startPos;
        ...
        var onScrollFn = function(event) {
            ...
            if (current.length) {
                ...
                if (lId !== cId) {
                    ...
                    deselectAll();
                    tMenu
                        .querySelector('[href="#' + cId + '"]')
                        .parentElement.classList.add(config.class);
                }

                if (getViewportH() + scrollY() >= root.scrollHeight) {
                    deselectAll();
                    navLinks[navLinks.length - 1].parentElement.classList.add(
                        config.class
                    );
                }
            }
...
function scrollTo(Y, duration, easingFunction, callback) {
    var start = Date.now(),
        root = /firefox|trident/i.test(navigator.userAgent)
            ? document.documentElement
            : document.body,
        elem = root,
        from = elem.scrollTop;
